...
//Total Loan Entry
$tot_le = "SELECT COALESCE(count(cp.id),0) AS total_entry FROM `customer_profile` cp JOIN loan_entry_loan_calculation lelc ON cp.id = lelc.cus_profile_id JOIN customer_status cs ON cp.id = cs.cus_profile_id WHERE cs.status > 1  ";
...
//Total Balance
$tot_bal = "SELECT COALESCE(count(cp.id),0) AS total_balance FROM `customer_profile` cp JOIN loan_entry_loan_calculation lelc ON cp.id = lelc.cus_profile_id JOIN customer_status cs ON cp.id = cs.cus_profile_id WHERE cs.status < 7 AND cs.status != 1 AND cs.status != 5 AND cs.status != 6  ";

//Today Loan Entry
$today_le = "SELECT COALESCE(count(cp.id),0) AS today_entry FROM `customer_profile` cp JOIN loan_entry_loan_calculation lelc ON cp.id = lelc.cus_profile_id JOIN customer_status cs ON cp.id = cs.cus_profile_id WHERE cs.status > 1 AND DATE(lelc.created_on) = CURDATE() ";
...
//Today Balance
$today_bal = "SELECT COALESCE(count(cp.id),0) AS today_balance FROM `customer_profile` cp JOIN loan_entry_loan_calculation lelc ON cp.id = lelc.cus_profile_id JOIN customer_status cs ON cp.id = cs.cus_profile_id WHERE cs.status < 7 AND cs.status != 1 AND cs.status != 5 AND cs.status != 6  AND DATE(cs.updated_on) = CURDATE() ";
