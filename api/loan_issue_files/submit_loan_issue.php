...
if ($payment_type == "1" && $payment_mode == '1') {
    ...
    if ($bal_amount == 0) {
        $qry = $pdo->query("UPDATE `customer_status` SET `status`='7',`coll_status`='Current',`payable_amnt`='$cus_payable',`bal_amnt`='$total_amnt_calc',`update_login_id`='$user_id',`updated_on`=now() WHERE `cus_profile_id`='$cus_profile_id' "); //Loan Issued.
    }
