...
$query = "SELECT cp.id, lelc.loan_date, cp.cus_id, cp.aadhar_num, cp.cus_name, anc.areaname, lnc.linename, bc.branch_name , cp.mobile1, lc.loan_category, lelc.loan_amount, cp.cus_data, lelc.id as loan_calc_id, cs.id as cus_sts_id, cs.status as c_sts ,fi.fam_name AS guarantor_name
...
WHERE cs.status = 4 AND cp.payment_mode_status = 1 AND u.id ='$user_id' AND us.id ='$user_id' $branchCondition $lineCondition $loanCatCondition";
