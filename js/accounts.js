$(document).ready(function () {
    ...
    $('#expenses_trans_id').blur(function () {
        checkTransactionCommon({ transInput: this, type: 'debit', bankId: $('#expenses_bank_name :selected').val(), dateField: '#expenses_trans_date' })
    });
    ...
    $('#submit_expenses_creation').click(function (event) {
        ...
        let expensesData = {
            ...
            'expenses_trans_date': $('#expenses_trans_date').val()
        }
            ...
            if (expensesFormValid(expensesData)) {
                $.post('api/accounts_files/accounts/submit_expenses.php', expensesData, function (response) {
                    ...
                    if (response.status == 'success') {
                        ...
                        swalError('Error', response.message || 'Failed to add expenses.');
                    }
                }, 'json');
            } else {
    ...
    $('#other_trans_id').blur(function () {
        let catType = $('#cat_type').val();
        let type = '';
        ...
        checkTransactionCommon({ transInput: this, type: type, bankId: $('#othertransaction_bank_name :selected').val(), dateField: '#other_trans_date' })
    });
    ...
    async function checkTransactionCommon(options) {
        ...
        try {
            ...
            $(dateField).val(response.data.trans_date);
    ...
    $('#submit_other_transaction').click(function (event) {
        ...
        let otherTransData = {
            ...
            'other_trans_date': $('#other_trans_date').val(),
                ...
                if (otherTransFormValid(otherTransData)) {
                    $.post('api/accounts_files/accounts/submit_other_transaction.php', otherTransData, function (response) {
                        if (response.status == 'success') {
                            ...
                            swalError('Error', response.message || 'Failed to add transaction.');
                        }
...
function getClosingBal(callback) {
    $.post('api/accounts_files/accounts/closing_balance.php', function (response) {
        if (response.length > 0) {
            let close1 = parseFloat($('.opening_val').text().replace(/,/g, '')) + parseFloat(response[0]['closing_balance']);
            let hand = parseInt($('.op_hand_cash_val').text().replace(/,/g, '')) + parseInt(response[0]['hand_cash']);
            let bank1 = parseFloat($('.op_bank_cash_val').text().replace(/,/g, '')) || 0;
            let bank2 = parseFloat(response[0]['bank_cash']) || 0;
            let bank = (bank1 + bank2).toFixed(2);
            let close = close1.toFixed(2);
...
function getCollectionList() {
    ...
    $.post('api/accounts_files/accounts/accounts_collection_list.php', { cash_type }, function (response) {
        let columnMapping = [
            'sno',
            'name',
            'linename',
...
function getLoanIssueList() {
    ...
    $.post('api/accounts_files/accounts/accounts_loan_issue_list.php', { cash_type }, function (response) {
        let columnMapping = [
            'sno',
            'name',
            'linename',
...
function expensesFormValid(expensesData) {
    for (key in expensesData) {
        if (key != 'agent_name' && key != 'expenses_total_issued' && key != 'expenses_total_amnt' && key != 'bank_id' && key != 'expenses_trans_id' && key != 'expenses_trans_date') {
    ...
    if (expensesData['coll_mode'] == '2') {
        if (expensesData['bank_id'] == '' || expensesData['bank_id'] == null || expensesData['bank_id'] == undefined || expensesData['expenses_trans_id'] == '' || expensesData['expenses_trans_id'] == null || expensesData['expenses_trans_id'] == undefined || expensesData['expenses_trans_date'] == '' || expensesData['expenses_trans_date'] == null || expensesData['expenses_trans_date'] == undefined) {
            return false;
        }
    }
...
function deleteExp(id) {
    ...
    $.post(
        'api/accounts_files/accounts/delete_expenses.php',
        { id: id },
        function (response) {
            ...
            if (response.status == 1) {
                ...
                swalError('Alert', response.message || 'Delete Failed');
            }
        ...
        .fail(function () {
            swalError('Error', 'Server Error. Please try again.');
        });
}
...
function otherTransFormValid(data) {
    for (key in data) {
        if (key != 'expenses_total_amnt' && key != 'bank_id' && key != 'other_trans_id' && key != 'other_trans_date') {
    ...
    if (data['coll_mode'] == '2') {
        if (data['bank_id'] == '' || data['bank_id'] == null || data['bank_id'] == undefined || data['other_trans_id'] == '' || data['other_trans_id'] == null || data['other_trans_id'] == undefined || data['other_trans_date'] == '' || data['other_trans_date'] == null || data['other_trans_date'] == undefined) {
            return false;
        }
    }
