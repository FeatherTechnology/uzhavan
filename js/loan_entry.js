...
const LOAN_ENTRY_FILTER_KEY = 'loan_entry_search_filters';
...
$(document).ready(function () {
    ...
    $('input[name=loan_entry_type]').click(function () {
        ...
        if (loanEntryType == 'cus_profile') {
            $('#loan_entry_customer_profile').show();
            $('#documentation_form').hide();
            $('#loan_entry_loan_calculation').hide();

        } else if (loanEntryType == 'loandoc' || loanEntryType == 'loan_calc') {
            // Before showing the form, check customer profile
            let cus_id = $('#auto_gen_cus_id').val();
            let cus_profile_id = $('#customer_profile_id').val();
            ...
            if (!cus_profile_id) {
                ...
                $.post('api/loan_entry/cus_sts_check.php', { cus_id, cus_profile_id }, function (response) {
                    ...
                    if (status == 0) {
                        ...
                        $('#loan_entry_customer_profile').hide();
                    }
                }, 'json');
            }
        }
        ...
        if (isValid) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('.cus_status_div').show();
                    // $('.loan_count_div').show();
                    getLoanCount(cus_id, response.last_id);
                    // }
                    // else {
                    // $('.loan_count_div').hide();
                    // }
...
//On Load function 
$(function () {
    let savedLoanFilters = getSavedLoanEntryFilters();
    if (savedLoanFilters) {
        restoreLoanEntryFilters(savedLoanFilters);
    }
    getLoanEntryTable();
...
function getLoanEntryTable() {
    ...
    serverSideTable('#loan_entry_table', params, 'api/loan_entry/loan_entry_list.php');
}
...
$('#submit_gold_info').click(function (event) {
    ...
    if (goldInfo.customer_profile_id === '' || goldInfo.customer_profile_id === null) {
        swalError('Warning', 'Kindly Fill the Personal Info');
        return false;
    }
...
function saveLoanEntryFilters() {
    ...
    sessionStorage.setItem(LOAN_ENTRY_FILTER_KEY, JSON.stringify(filters));
}

function getSavedLoanEntryFilters() {
    let saved = sessionStorage.getItem(LOAN_ENTRY_FILTER_KEY);
...
function restoreLoanEntryFilters(filters) {
